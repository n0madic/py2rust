...
def test_custom_ioerror_inheritance():
    ...
    try:
        ...
        caught_network = True
    assert caught_network, "TimeoutError should be caught by NetworkError handler"
...
# ===== SECTION: Regression coverage imported from previous runtime/exceptions.py =====

def catch_with_exception(exc_type: int) -> bool:
    """Catch different concrete errors with a single `except Exception`."""
    try:
        if exc_type == 1:
...
def test_exception_catch_all() -> None:
    """Regression: catch-all Exception handler should catch multiple error types."""
...
print("test_exception_catch_all passed")

def catch_and_reraise() -> None:
    """Re-raise from a catch-all block and keep the dynamic exception type."""
    try:
...
def test_catch_reraise() -> None:
    """Regression: `except Exception` + bare raise should preserve ValueError."""
    caught: bool = False
    try:
        catch_and_reraise()
    except ValueError:
...
test_catch_reraise()
print("test_catch_reraise passed")

def test_not_implemented() -> None:
    """Regression: explicit NotImplementedError raise/catch."""
    caught: bool = False
    try:
...
print("test_not_implemented passed")

def test_builtin_errors() -> None:
    """Regression: builtin operations should raise expected exception types."""
    caught: bool = False
    try:
        chr(-1)
    except ValueError:
...
test_builtin_errors()
print("test_builtin_errors passed")

# Top-level exception handling regression checks.
...
print("top_level_exception_handling passed")

def test_exception_narrowing() -> None:
    """Regression: handler matching should be type-specific and ordered."""
...
print("test_exception_narrowing passed")

def test_try_partial_value_return() -> None:
    """Regression: try-path early return and fallthrough path must both work."""
...
print("test_try_partial_value_return passed")

def raises_value_error() -> int:
    """Helper used to verify propagation and narrow matching across calls."""
...
def test_propagation_narrowing() -> None:
    """Regression: propagated exceptions should match only compatible handlers."""
    caught: bool = False
    try:
        raises_value_error()
    except ValueError:
