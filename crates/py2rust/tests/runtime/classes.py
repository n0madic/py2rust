...
assert (
    AnnotatedClassAttr.increment_count() == 1
), "AnnotatedClassAttr.increment_count() should equal 1"
...
# Keep the legacy completion signal as an assertion instead of print output.
assert ClassMethodBasic.get_count() == 3, "ClassMethodBasic.get_count() should equal 3"

# ===== SECTION: __match_args__ support =====
...
MatchCoord = MatchPoint | MatchColor


def describe_match_coord(c: MatchCoord) -> str:
    match c:
...
mp: MatchCoord = MatchPoint(10, 20)
mc: MatchCoord = MatchColor(255, 128, 64)
assert (
    describe_match_coord(mp) == "Point: y=20, x=10"
), "describe_match_coord(MatchPoint(10, 20)) mismatch"
assert (
    describe_match_coord(mc) == "Color: r=255, g=128, b=64"
), "describe_match_coord(MatchColor(255, 128, 64)) mismatch"

# ===== SECTION: __match_args__ subset =====
...
MatchShape = MatchRect


def match_area(s: MatchShape) -> int:
    match s:
        case MatchRect(w, h):
